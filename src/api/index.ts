...
export const SearchMovies = async (query: any, type?: any, pages = 1) =>{
  const {results, page, total_pages}: Results = await fetch(API_Search_URL(type,query,pages)).then(x => x.json());
